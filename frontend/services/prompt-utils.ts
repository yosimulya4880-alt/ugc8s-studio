...
export function buildGeneratedPrompt(state: PromptGeneratorState): string {
  ...
  if (state.lang === "id") {
    return [
      ...
      state.extraDetails ? `Detail tambahan: ${state.extraDetails}.` : "",
  ...
  return [
    ...
    state.mood ? `Mood: ${state.mood}.` : "",
    state.extraDetails ? `Additional details: ${state.extraDetails}.` : "",
